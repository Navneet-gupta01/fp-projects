...
final case class ArticleEntity(slug: String,
                               title: String,
                               description: String,
                               body: String,
                               id: Option[Long] = None)
...
final case class ArticleForm(description: Option[String],
                             body: Option[String],
                             title: Option[String],
                             tagList: Option[List[String]],
                             slug: Option[String])
final case class CreateArticleForm(description: String,
                                   body: String,
                                   title: String,
                                   slug: String,
                                   tags: List[Tags])
final case class UpdateArticleForm(description: Option[String],
                                   body: Option[String],
                                   title: Option[String],
                                   slug: String)
...
object ArticleForm {
  ...
  val validateDescription =
    notEmptyString("Description is invalid, Should be not Empty")(_)
  ...
  def createArticleForm(
      articleForm: ArticleForm): Validated[CreateArticleForm] =
    (validateDescription(articleForm.description),
     validateBody(articleForm.body),
     validateTitle(articleForm.title)).mapN(
      (a, b, c) =>
        CreateArticleForm(a,
                          b,
                          c,
                          createSlug(c),
                          articleForm.tagList.getOrElse(List()).map(Tags(_))))

  def updateArticleForm(
      articleForm: ArticleForm): Validated[UpdateArticleForm] =
    validateSlug(articleForm.slug.getOrElse("")).map(
      a =>
        UpdateArticleForm(articleForm.description,
                          articleForm.body,
                          articleForm.title,
                          a))

  private def createSlug(str: String): String =
...
trait BaseArticleResponse {
  def slug: String
  def description: String
  def title: String
  def body: String
  def author: ProfileEntity
  def tagList: List[String]
  def createdAt: Date
  def updatedAt: Date
  def favorited: Boolean
  def favoritesCount: Integer
}
final case class ArticleResponse(slug: String,
    ...
    extends BaseArticleResponse

object ArticleResponse {
  def apply(articleEntity: ArticleEntity,
            created_at: Date,
            updated_at: Date,
            username: String,
            bio: Option[String],
            image: Option[String],
            followee_id: Option[Long]): ArticleResponse =
    ArticleResponse(
      articleEntity.slug,
  ...
  def apply(
      queryResp: (ArticleEntity,
                  Date,
                  Date,
                  String,
                  Option[String],
                  Option[String],
                  Option[Long])): ArticleResponse = {
    ...
    ArticleResponse(
      ...
      articleEntity.title,
      articleEntity.body,
      ProfileEntity(queryResp._4,
                    queryResp._5,
                    queryResp._6,
                    queryResp._7.fold(false)(_ => true)),
      List(),
      queryResp._2,
      queryResp._3
    )
...
final case class ArticleResponseWithId(id: Long,
                                       slug: String,
                                       description: String,
                                       title: String,
                                       body: String,
