...
object ArticlesQueries {
  ...
  def getOwnedyArticleQuery(slug: String, author_id: Long) : Query0[ArticleEntity] =
    sql"""SELECT slug, title, description, body, id from articles where slug = ${slug} and author_id= ${author_id}""".query[ArticleEntity]

  def getArticleBySlugQuery(slug: String): Query0[ArticleEntity] =
    sql"""SELECT slug, title, description, body, id from articles where slug = ${slug}""".query[ArticleEntity]
  ...
  def getFollowedArticleQuery(limit: Long, offset: Long, userId: Long): Query0[(ArticleEntity, Date, Date, String, Option[String], Option[String], Option[Long])] =
    sql"""
          SELECT ar.slug,ar.title,ar.description,ar.body,ar.id,ar.created_at, ar.updated_at, ac.username, ac.bio, ac.image, u.followee_id from accounts ac
          INNER join users_followers u on u.followee_id = ac.id and u.follower_id = ${userId}
          INNER JOIN articles ar on ar.author_id = ac.id order by ar.created_at limit $limit offset $offset
